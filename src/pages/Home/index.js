...
const Page = () => {
  const { data } = useData(); // Assurez-vous que `data` contient les événements

  // Trouver le dernier événement si les données sont disponibles
  const latestEvent = data ? getLatestEvent(data.events) : null;
